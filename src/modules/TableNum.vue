    ...
    <el-table :data="tableData" style="width: 100%" border>
      <el-table-column label="" width="180">
        <template slot-scope="scope">
          <span v-if="scope.$index !== tableData.length - 1">第{{ numberArr[scope.$index] }}行</span>
          <span v-else>合计</span>
        </template>
      </el-table-column>
      <el-table-column label="第一列" width="180">
        <template slot-scope="scope">
          <el-input v-model="scope.row.value1" :disabled="scope.$index === tableData.length - 1"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="第二列" width="180">
        <template slot-scope="scope">
          <el-input v-model="scope.row.value2" :disabled="scope.$index === tableData.length - 1"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="第三列" width="180">
        <template slot-scope="scope">
          <el-input v-model="scope.row.value3" :disabled="scope.$index === tableData.length - 1"></el-input>
        </template>
      </el-table-column>
...
export default class TableNum extends Vue {
  ...
  tableDataChange(newVal: any) {
    ...
    for (let i = 0, l = newVal.length - 1; i < l; i++) {
      ...
      item.value4 = Number(item.value1) + Number(item.value2) + Number(item.value3)
    }
